...
bool DynamixelServoHardwareInterface::init() {
  ...
  */
  ...
  RCLCPP_ERROR_STREAM(nh_->get_logger(), "Found joint count: " << std::to_string(joint_names_.size()));
...
void DynamixelServoHardwareInterface::commandCb(const bitbots_msgs::msg::JointCommand &command_msg) {
  if (0 || !(command_msg.joint_names.size()==command_msg.positions.size() &&
  ...
  for (unsigned int i = 0; i < command_msg.joint_names.size(); i++) {
    int joint_id = joint_map_[command_msg.joint_names[i]];// int joint_id = 0;
