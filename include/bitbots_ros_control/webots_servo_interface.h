...
class WebotsServoHardwareInterface : public bitbots_ros_control::HardwareInterface{
 public:
  explicit WebotsServoHardwareInterface(rclcpp::Node::SharedPtr nh);
...
private:
  ...
  std::vector<double> goal_effort_; // not using
  std::vector<double> goal_velocity_; // not using
  std::vector<double> goal_acceleration_; // not using

  std::vector<double> current_position_;
  std::vector<double> current_velocity_; // not using
  std::vector<double> current_effort_; // not using
  std::vector<double> current_pwm_; // not using
  std::vector<double> current_input_voltage_; // not using 
  std::vector<double> current_temperature_; // not using
  std::vector<uint8_t> current_error_;

  std::map<std::string, int> joint_map_;

  bool torqueless_mode_; // not using 
  ...
  rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr pwm_pub_; // not using
